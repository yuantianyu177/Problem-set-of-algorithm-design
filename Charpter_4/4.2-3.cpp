/* Multiplication of matrices, Strassen algorithm ADAPTS to non-quadratic powers */
int** my_malloc(int n) {
	int** A = new int* [n];
	for (int i = 0; i < n; i++) {
		A[i] = new int[n];
...
int** Strassen(int* A[], int* B[], int n) {
	int zeroLines;
	zeroLines = get_zeroLine(n);
	int** A_extend = my_malloc(n + zeroLines);
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < n; j++) {
			A_extend[i][j] = A[i][j];
		}
	}
	int** B_extend = my_malloc(n + zeroLines);
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < n; j++) {
			B_extend[i][j] = B[i][j];
		}
	...
	n = n + zeroLines;
	int** A11 = init(A_extend, 1, 1, n);
	int** A12 = init(A_extend, 1, 2, n);
	int** A21 = init(A_extend, 2, 1, n);
	int** A22 = init(A_extend, 2, 2, n);
	int** B11 = init(B_extend, 1, 1, n);
	int** B12 = init(B_extend, 1, 2, n);
	int** B21 = init(B_extend, 2, 1, n);
	int** B22 = init(B_extend, 2, 2, n);
